...
function mergeWebsite() {
    ...
    if (websiteData.orgId !== "") {
        ...
        if ((!websiteData.general.favicon && websiteData.general.favicon !== "") || !websiteData.general.name) {
            throw new Error(`general must have "favicon" and "name" attributes.`);
        }

        if (!websiteData.font.cssRule || !websiteData.font.href || !websiteData.font.id || !websiteData.font.name) {
    ...
    const newValidationJsonString = JSON.stringify({ ...previousValidation, ...websiteValidation }, null, 2);
    ...
    fs.writeFileSync(path.join(__dirname, `../public/assets/data/${_FILES.website}`), websiteJsonString);
    console.log(logSymbols.success, `\x1b[32m`, `${_FILES.website}`, `\x1b[0m`, `merged data successfully.`);

    //create/change config
    const configJsonString = JSON.stringify(configData, null, 2);
    fs.writeFileSync(path.join(__dirname, `../public/assets/data/${_FILES.config}`), configJsonString);
    console.log(logSymbols.success, `\x1b[32m`, `${_FILES.config}`, `\x1b[0m`, `merged config successfully.`);
}

module.exports = mergeWebsite;
