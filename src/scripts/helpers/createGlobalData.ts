...
export async function createGlobalData({
  ...
  if (typeof globalModule.default === "function") {
    ...
    fs.access(process.env.APP_PUBLIC_DATA, async (err) => {
      if (err) {
        await fs.mkdirSync(process.env.APP_PUBLIC_DATA, { recursive: true });
      }
      await fs.writeFileSync(
        path.join(process.env.APP_PUBLIC_DATA, `config.website.json`),
        configWebsiteJson
      );
      await fs.writeFileSync(
        path.join(process.env.APP_PUBLIC_DATA, `validation.website.json`),
        validationWebsiteJson
      );
      await fs.writeFileSync(
        path.join(process.env.APP_PUBLIC_DATA, `global.website.json`),
        globalWebsiteJson
      );
    });

    //delete js module after generate json file in public folder.
    await fs.unlinkSync(path.resolve(outputPath, globalModule.name));
  } else {
