...
export async function createPagesData({
  ...
  const {
    ...
    for (let i = 0; i < pagesModules.length; i++) {
      if (typeof pagesModules[i].default === "function") {
         ...
         */
        const sectionsValidation = sections.validation.filter((section) =>
          result.data.withSections.includes(section.name)
        );
  ...
  fs.access(process.env.APP_PUBLIC_DATA, async (err) => {
    ...
    await fs.writeFileSync(
      path.join(process.env.APP_PUBLIC_DATA, `pages.website.json`),
      jsonResult
    );
  });
