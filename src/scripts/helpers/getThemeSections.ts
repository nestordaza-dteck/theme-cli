 ...
 */
export async function getThemeSections({ assets, outputPath }: { assets: webpack.StatsAsset[]; outputPath: string }) {
    ...
    for (let i = 0; i < sectionModules.length; i++) {
        if (typeof sectionModules[i].default === "function") {
            ...
            //delete js module.
            fs.unlinkSync(path.resolve(outputPath, sectionModules[i].name));
        } else {
