...
const createDeclarationFiles = async (options: CLIOptions) => {
    try {
        const srcPath = path.join(`${options.targetDirectory}/${clearThemeName(options.projectName)}`, "src");
        ...
        const dataDeclaration = fs.readFileSync(path.join(__dirname, `${_DATA_FILE_NAME}`));
        const pluginDeclaration = fs.readFileSync(path.join(__dirname, `${_PLUGINS_FILE_NAME}`));
        const themeDeclaration = fs.readFileSync(path.join(__dirname, `${_THEME_FILE_NAME}`));
        ...
        for (let i = 0; i < declarationListStringKeys.length; i++) {
            ...
            fs.access(srcPath, async (err) => {
                if (err) {
                    fs.mkdirSync(srcPath, { recursive: true });
                }
                fs.writeFileSync(path.join(srcPath, fileName), declarationListString[fileName]);
            });
        }
    } catch (error) {
        return Promise.reject(new Error(`Failed to create declaration files, reason: ${error}`));
    }
};
...
async function setPackageInfoDefaults(options: CLIOptions) {
    try {
        const packageJsonPath = path.join(`${options.targetDirectory}/${clearThemeName(options.projectName)}`, "package.json");
        if (path.join(packageJsonPath, "package.json")) {
            ...
            defaultPackageJson["browserslist"] = ["defaults", "not ie < 11", "last 2 versions", "> 1%", "iOS 7", "last 3 iOS versions"];
            fs.writeFileSync(packageJsonPath, JSON.stringify(defaultPackageJson, null, 2));
        }
    } catch (error) {
        return Promise.reject(
            new Error('Something went wrong while setting up package.json defaults, try to delete the folder and re-run the "create-soltivo-theme".')
        );
    }
}
...
async function createInitialFile(options: CLIOptions) {
    try {
        ...
        //.env
        fs.writeFileSync(
            path.join(`${options.targetDirectory}/${clearThemeName(options.projectName)}`, ".env"),
        ...
        //.gitignore
        fs.writeFileSync(
            path.join(`${options.targetDirectory}/${clearThemeName(options.projectName)}`, ".gitignore"),
            `node_modules/\nbuild/\ndist/\npackage-lock.json\nyarn-lock.json
        `
        );
...
async function copyTemplateFiles(options: CLIOptions) {
    ...
    const alreadyExists = fs.existsSync(`${options.targetDirectory}/${clearThemeName(options.projectName)}`);
    if (alreadyExists) {
        return Promise.reject(new Error("Failed to create project, folder directory already exists."));
    }

    return copy(options.templateDirectory, `${options.targetDirectory}/${clearThemeName(options.projectName)}`, {
        clobber: false,
    });
}
...
export async function createProject(options: CLIOptions) {
    ...
    console.log(
        ...
        `🔷 Run ${chalk.blueBright.bold(`cd ${clearThemeName(options.projectName)}`)} to enter your project directory`,
        `🔷 Run ${chalk.blueBright.bold("yarn start")} or ${chalk.blueBright.bold("npm run start")} to start the development server.`,
        `🔷 Run ${chalk.blueBright.bold("yarn build")} or ${chalk.blueBright.bold("npm run build")} to build a production version of your project.`,
        chalk.blueBright.bold(`🔥 Make something beautiful.`)
    );
